...
// NOTE: Blockchain token operations (mint, transfer, burn) are handled by the frontend via web3 microservice
export const getCreditService = () => {
  ...
  // Initialize wallet repository to fetch wallet addresses (for future COT token operations)
  const walletRepository = new LinkedWalletRepository();

  logger.info('CreditService initialized (web2 only - token operations via web3 microservice)');
...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      // Retire credits (Requirements 7.1-7.8)
      // NOTE: Token burning is handled by the frontend via web3 microservice
      const { creditEntry, transaction } = await creditService.retireCredits(
        creditId,
        userId,
        quantity,
        reason
