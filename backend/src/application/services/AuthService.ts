...
export class AuthService {
   ...
   * 
   * NOTE: Wallet signature verification should be handled by the frontend/web3 microservice
   * The frontend should verify the signature before calling this endpoint
   * This method trusts the frontend's verification and focuses on session creation
   */
  async verifyWallet(
    _challengeId: string,
    address: string,
    ...
    // NOTE: Signature verification is now handled by the frontend via web3 microservice
    // Frontend should call POST /api/web3/wallet/verify-signature before this endpoint
    // This endpoint assumes the signature has already been verified by the frontend

    logger.info('Wallet authentication attempt', { address });
