...
export class VerificationService {
  constructor(
    ...
    private validatorVoteRepository?: IValidatorVoteRepository
  ) { }
  ...
  // ============================================================================
  // MULTIVALIDATOR METHODS
  // ============================================================================

  /**
   * Assign multiple validators to a verification request
   * Enables the multivalidator system with consensus-based approval
   */
  async assignMultipleValidators(
    verificationId: string,
    validatorIds: string[],
    requiredApprovals: number,
    assignedBy: string,
    assignedByRole: UserRole
  ): Promise<{ verification: VerificationRequest; assignments: ValidatorAssignment[] }> {
    // Validate user is administrator
    if (assignedByRole !== UserRole.ADMINISTRATOR) {
      throw new Error('Only administrators can assign validators');
    }

    if (!this.validatorAssignmentRepository || !this.validatorVoteRepository) {
      throw new Error('Multivalidator system is not enabled');
    }
    ...
    // Validate verification is in pending status
    if (verification.status !== VerificationStatus.PENDING) {
      throw new Error('Can only assign validators to pending verifications');
    }

    // Validate validator count
    if (validatorIds.length < requiredApprovals) {
      throw new Error(
        `Number of validators (${validatorIds.length}) must be >= required approvals (${requiredApprovals})`
      );
    }

    if (validatorIds.length > 10) {
      throw new Error('Cannot assign more than 10 validators');
    }

    // Remove duplicates
    const uniqueValidatorIds = [...new Set(validatorIds)];

    // Validate all validators exist and have correct role
    const validators = await Promise.all(
      uniqueValidatorIds.map((id) => this.userRepository.findById(id))
    );

    for (let i = 0; i < validators.length; i++) {
      const validator = validators[i];
      if (!validator) {
        throw new Error(`Validator ${uniqueValidatorIds[i]} not found`);
      }
      if (validator.role !== UserRole.VERIFIER && validator.role !== UserRole.ADMINISTRATOR) {
        throw new Error(`User ${validator.name} must have verifier or administrator role`);
      }
    }

    // Delete any existing assignments and votes
    await this.validatorAssignmentRepository.deleteAllForVerification(verificationId);
    ...
    // Update verification to use multivalidator system
    verification.status = VerificationStatus.IN_REVIEW;
    verification.progress = 30;
    verification.assignedAt = new Date();
    verification.useMultivalidator = true;
    verification.requiredApprovals = requiredApprovals;
    verification.approvalCount = 0;
    verification.rejectionCount = 0;
    verification.voteCount = 0;
    ...
    const timelineEvent: VerificationEvent = {
      id: uuidv4(),
      verificationId: verification.id,
      eventType: 'validators_assigned',
      message: `${uniqueValidatorIds.length} validators assigned (${requiredApprovals} approvals required for consensus)`,
      userId: assignedBy,
      metadata: {
        validatorIds: uniqueValidatorIds,
        validatorCount: uniqueValidatorIds.length,
        requiredApprovals,
    ...
    try {
      const projectName = 'the project'; // We'll fetch this if needed
      await Promise.all(
        validators.map((validator) =>
          this.emailService.sendNotificationEmail(
            validator!.email,
            'New Verification Assignment - Karbonica',
            this.generateValidatorAssignmentEmail(
              validator!.name,
              projectName,
              verificationId,
              uniqueValidatorIds.length,
              requiredApprovals
            )
          )
        )
      ...
      logger.info('Validator assignment notifications sent', {
        verificationId,
        validatorCount: uniqueValidatorIds.length,
      });
    } catch (error) {
      logger.error('Failed to send validator assignment notifications', {
        error,
        verificationId,
      });
      // Don't fail the assignment if email fails
    ...
    logger.info('Validators assigned to verification', {
      verificationId,
      validatorCount: uniqueValidatorIds.length,
      requiredApprovals,
      assignedBy,
    ...
    return { verification: updatedVerification, assignments };
  }
  ...
  async castVote(
  ...
  ): Promise<{ vote: ValidatorVote; consensusStatus: VerificationConsensusStatus }> {
    if (!this.validatorAssignmentRepository || !this.validatorVoteRepository) {
      throw new Error('Multivalidator system is not enabled');
    }
    ...
    // Validate verification uses multivalidator system
    if (!verification.useMultivalidator) {
      throw new Error('This verification does not use the multivalidator system');
    }

    // Validate verification is in review status
    if (verification.status !== VerificationStatus.IN_REVIEW) {
      throw new Error('Verification must be in review status to cast votes');
    }

    // Validate validator is assigned to this verification
    const isAssigned = await this.validatorAssignmentRepository.isValidatorAssigned(
      verificationId,
      validatorId
    );

    if (!isAssigned && userRole !== UserRole.ADMINISTRATOR) {
      throw new Error('You are not assigned to this verification');
    }

    // Save the vote (upsert) with wallet signature if provided
    const validatorVote = await this.validatorVoteRepository.save({
      verificationId,
      validatorId,
      vote,
      notes,
    ...
    // Get validator name for timeline event
    const validator = await this.userRepository.findById(validatorId);
    const validatorName = validator?.name || 'Unknown';

    // Create timeline event
    const voteText = vote === VoteDecision.APPROVE ? 'approved' : vote === VoteDecision.REJECT ? 'rejected' : 'abstained';
    const timelineEvent: VerificationEvent = {
      id: uuidv4(),
      verificationId: verification.id,
      eventType: 'validator_voted',
      message: `Validator ${validatorName} ${voteText}${notes ? `: ${notes}` : ''}`,
      userId: validatorId,
      metadata: {
        vote,
        validatorId,
    ...
    const consensusStatus = await this.validatorVoteRepository.getConsensusStatus(verificationId);

    // Check if consensus has been reached and handle automatic approval/rejection
    if (consensusStatus.consensusReached) {
      await this.handleConsensusReached(verification, consensusStatus, validatorId);
    }

    logger.info('Validator vote cast', {
      verificationId,
      validatorId,
      vote,
    ...
    return { vote: validatorVote, consensusStatus };
  }
  ...
  async getConsensusStatus(verificationId: string): Promise<VerificationConsensusStatus> {
    if (!this.validatorVoteRepository) {
      throw new Error('Multivalidator system is not enabled');
    }

    return await this.validatorVoteRepository.getConsensusStatus(verificationId);
  }

  /**
   * Get all validator assignments for a verification
   */
  async getValidatorAssignments(verificationId: string): Promise<ValidatorAssignment[]> {
    if (!this.validatorAssignmentRepository) {
      throw new Error('Multivalidator system is not enabled');
    }

    return await this.validatorAssignmentRepository.findByVerification(verificationId);
  }

  /**
   * Get all votes for a verification
   */
  async getValidatorVotes(verificationId: string): Promise<ValidatorVote[]> {
    if (!this.validatorVoteRepository) {
      throw new Error('Multivalidator system is not enabled');
    }

    return await this.validatorVoteRepository.findByVerification(verificationId);
  }

  /**
   * Handle consensus reached - automatically approve or reject verification
   */
  private async handleConsensusReached(
    verification: VerificationRequest,
    consensusStatus: VerificationConsensusStatus,
    triggeredBy: string
  ): Promise<void> {
    ...
    if (consensusStatus.finalDecision === 'approved') {
      // Auto-approve based on consensus
      await this.handleMultivalidatorApproval(verification, triggeredBy);
    } else if (consensusStatus.finalDecision === 'rejected') {
      // Auto-reject based on consensus
      await this.handleMultivalidatorRejection(verification, triggeredBy);
    }
  }

  /**
   * Handle multivalidator approval (when consensus is reached)
   */
  private async handleMultivalidatorApproval(
    verification: VerificationRequest,
    triggeredBy: string
  ): Promise<void> {
    // For multivalidator system, validators ARE the verification
    // Document requirement is optional since validators sign transactions
    const documents = await this.verificationDocumentRepository.findByVerification(verification.id);

    logger.info('Multivalidator approval triggered', {
      verificationId: verification.id,
      documentCount: documents.length,
      triggeredBy,
    });
    ...
    if (this.projectRepository) {
      ...
      if (project) {
        ...
        logger.info('Project status updated to verified (multivalidator)', {
          projectId: project.id,
          verificationId: verification.id,
        });
        ...
        if (this.creditService) {
          try {
            ...
            logger.info('Credits issued automatically on multivalidator consensus', {
              verificationId: verification.id,
              projectId: verification.projectId,
              creditId: creditEntry.creditId,
              quantity: creditEntry.quantity,
            ...
            // Create timeline event for credit issuance
            const creditEvent: VerificationEvent = {
              id: uuidv4(),
              verificationId: verification.id,
              eventType: 'credits_issued',
              message: `Carbon credits issued: ${creditEntry.quantity} tons CO2e (Serial: ${creditEntry.creditId})`,
            ...
            await this.verificationEventRepository.save(creditEvent);
          } catch (error) {
            logger.error('Failed to issue credits on multivalidator consensus', {
              error,
              verificationId: verification.id,
            });
          }
    ...
    const timelineEvent: VerificationEvent = {
      ...
      verificationId: verification.id,
      eventType: 'verification_approved',
      message: 'Verification approved by consensus',
      userId: triggeredBy,
      metadata: {
        status: 'approved',
        approvalType: 'multivalidator_consensus',
        progress: 100,
      },
      createdAt: new Date(),
    };
    ...
    try {
      ...
      if (developer) {
        await this.emailService.sendNotificationEmail(
          developer.email,
          'Verification Approved - Karbonica',
          this.generateVerificationApprovedEmail(developer.name, verification.id)
        );
      }
    } catch (error) {
      logger.error('Failed to send multivalidator approval notification', {
        error,
        verificationId: verification.id,
      });
    }

    logger.info('Verification approved by multivalidator consensus', {
      verificationId: verification.id,
      projectId: verification.projectId,
    });
  }
  ...
  /**
   * Handle multivalidator rejection (when consensus is reached)
   */
  private async handleMultivalidatorRejection(
    verification: VerificationRequest,
    triggeredBy: string
  ): Promise<void> {
    // Collect rejection notes from all reject votes
    if (this.validatorVoteRepository) {
      const votes = await this.validatorVoteRepository.findByVerification(verification.id);
      const rejectionNotes = votes
        .filter((v) => v.vote === VoteDecision.REJECT)
        .map((v) => v.notes)
        .filter((n) => n)
        .join('; ');

      verification.notes = rejectionNotes || 'Rejected by validator consensus';
    }
    ...
    const timelineEvent: VerificationEvent = {
      ...
      verificationId: verification.id,
      eventType: 'verification_rejected',
      message: 'Verification rejected by consensus',
      userId: triggeredBy,
      metadata: {
        status: 'rejected',
        rejectionType: 'multivalidator_consensus',
        progress: 100,
      },
      createdAt: new Date(),
    };
    ...
    try {
      ...
      if (developer) {
        await this.emailService.sendNotificationEmail(
          developer.email,
          'Verification Rejected - Karbonica',
          this.generateVerificationRejectedEmail(
            developer.name,
            verification.id,
            verification.notes || 'Rejected by validator consensus'
          )
        );
      }
    } catch (error) {
      logger.error('Failed to send multivalidator rejection notification', {
        error,
        verificationId: verification.id,
      });
    }

    logger.info('Verification rejected by multivalidator consensus', {
      verificationId: verification.id,
      projectId: verification.projectId,
    });
  }

  private generateValidatorAssignmentEmail(
    ...
    return `
      ...
      <body>
        <div class="container">
          ...
          <div class="content">
            <h2>Hi ${validatorName},</h2>
            <p>You have been assigned as one of <strong>${totalValidators} validators</strong> to verify a carbon offset project.</p>

            <div class="info-box">
              <strong>Verification ID:</strong> ${verificationId}<br>
              <strong>Project:</strong> ${projectName}<br>
              <strong>Status:</strong> In Review<br>
              <strong>Your Role:</strong> Validator (${totalValidators} total)
            </div>

            <div class="consensus-box">
              <strong>⚖️ Consensus Requirement:</strong><br>
              This verification requires <strong>${requiredApprovals} out of ${totalValidators}</strong> approvals to pass.<br>
              Your vote matters!
            </div>
            ...
            <ul>
              ...
              <li>Upload any additional verification documents if needed</li>
              <li>Cast your vote: Approve, Reject, or Abstain</li>
              <li>Add notes explaining your decision</li>
            </ul>

            <p><strong>Voting Options:</strong></p>
            <ul>
              <li><strong>Approve:</strong> Project meets verification criteria</li>
              <li><strong>Reject:</strong> Project does not meet criteria (provide detailed notes)</li>
              <li><strong>Abstain:</strong> You cannot make a decision (does not count toward consensus)</li>
            </ul>

            <div style="text-align: center;">
              <a href="${process.env.FRONTEND_URL || 'http://localhost:3000'}/verifications/${verificationId}" class="button">Review & Vote</a>
            </div>

            <p>The verification will be automatically approved or rejected when consensus is reached.</p>
          </div>
          <div class="footer">
  ...
  // NOTE: NFT state transition recording is handled by the web3 microservice
  // The frontend will call POST /api/web3/nft/transition which handles:
  // - Building the transaction
  // - Recording the state transition
  // - Updating the project NFT metadata
  // The backend only tracks the nft_state field on projects table
