...
export class VerificationRequestRepository implements IVerificationRequestRepository {
  ...
  async findById(id: string): Promise<VerificationRequest | null> {
    const query = `
      SELECT
        ...
        required_approvals as "requiredApprovals", approval_count as "approvalCount",
        rejection_count as "rejectionCount", vote_count as "voteCount",
        consensus_reached_at as "consensusReachedAt", use_multivalidator as "useMultivalidator",
        voting_deadline as "votingDeadline", auto_assign_validators as "autoAssignValidators",
        deadline_extended as "deadlineExtended", original_deadline as "originalDeadline"
  ...
  async update(verification: VerificationRequest): Promise<VerificationRequest> {
    const query = `
      ...
      SET
        ...
        rejection_count = $11,
        vote_count = $12,
        consensus_reached_at = $13,
        use_multivalidator = $14,
        voting_deadline = $15,
        auto_assign_validators = $16,
        deadline_extended = $17,
        original_deadline = $18
      WHERE id = $1
      RETURNING
        id, project_id as "projectId", developer_id as "developerId",
        verifier_id as "verifierId", status, progress,
    ...
    const values = [
      ...
      verification.voteCount,
      verification.consensusReachedAt,
      verification.useMultivalidator,
      verification.votingDeadline,
      verification.autoAssignValidators,
      verification.deadlineExtended,
      verification.originalDeadline,
    ];
  ...
  private mapRowToVerificationRequest(row: any): VerificationRequest {
    return {
      ...
      requiredApprovals: row.requiredApprovals ? parseInt(row.requiredApprovals) : 3,
      approvalCount: row.approvalCount ? parseInt(row.approvalCount) : 0,
      rejectionCount: row.rejectionCount ? parseInt(row.rejectionCount) : 0,
      voteCount: row.voteCount ? parseInt(row.voteCount) : 0,
      consensusReachedAt: row.consensusReachedAt ? new Date(row.consensusReachedAt) : null,
      useMultivalidator: row.useMultivalidator || false,
      votingDeadline: row.votingDeadline ? new Date(row.votingDeadline) : null,
      autoAssignValidators: row.autoAssignValidators !== undefined ? row.autoAssignValidators : true,
      deadlineExtended: row.deadlineExtended || false,
      originalDeadline: row.originalDeadline ? new Date(row.originalDeadline) : null,
    };
  }
}
