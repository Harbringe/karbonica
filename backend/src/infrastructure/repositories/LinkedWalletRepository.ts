...
/**
 * LinkedWalletRepository Implementation
 *
 * PostgreSQL implementation of the linked wallet repository.
 * This is a Web2 repository - it handles database operations,
 * not blockchain interactions.
 * 
 * Requirements: 2.3, 2.4, 2.8, 2.9
 */

export class LinkedWalletRepository implements ILinkedWalletRepository {
    private pool: Pool;
    ...
    async findById(id: string): Promise<LinkedWallet | null> {
        const query = `
      SELECT 
    ...
    async findByAddress(address: string): Promise<LinkedWallet | null> {
        const query = `
      SELECT 
        id, user_id, address, stake_address, public_key,
        linked_at, last_verified_at, is_active, created_at
    ...
    async save(wallet: LinkedWallet): Promise<LinkedWallet> {
        const query = `
    ...
    async update(wallet: LinkedWallet): Promise<LinkedWallet> {
        const query = `
    ...
    private mapRowToWallet(row: any): LinkedWallet {
        return {
            id: row.id,
            userId: row.user_id,
            address: row.address,
